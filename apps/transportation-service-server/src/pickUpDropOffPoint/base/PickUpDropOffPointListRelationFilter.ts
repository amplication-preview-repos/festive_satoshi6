/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PickUpDropOffPointWhereInput } from "./PickUpDropOffPointWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class PickUpDropOffPointListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => PickUpDropOffPointWhereInput,
  })
  @ValidateNested()
  @Type(() => PickUpDropOffPointWhereInput)
  @IsOptional()
  @Field(() => PickUpDropOffPointWhereInput, {
    nullable: true,
  })
  every?: PickUpDropOffPointWhereInput;

  @ApiProperty({
    required: false,
    type: () => PickUpDropOffPointWhereInput,
  })
  @ValidateNested()
  @Type(() => PickUpDropOffPointWhereInput)
  @IsOptional()
  @Field(() => PickUpDropOffPointWhereInput, {
    nullable: true,
  })
  some?: PickUpDropOffPointWhereInput;

  @ApiProperty({
    required: false,
    type: () => PickUpDropOffPointWhereInput,
  })
  @ValidateNested()
  @Type(() => PickUpDropOffPointWhereInput)
  @IsOptional()
  @Field(() => PickUpDropOffPointWhereInput, {
    nullable: true,
  })
  none?: PickUpDropOffPointWhereInput;
}
export { PickUpDropOffPointListRelationFilter as PickUpDropOffPointListRelationFilter };
