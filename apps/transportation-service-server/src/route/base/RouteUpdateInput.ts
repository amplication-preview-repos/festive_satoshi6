/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsNumber,
  IsOptional,
  IsString,
  ValidateNested,
} from "class-validator";
import { PickUpDropOffPointUpdateManyWithoutRoutesInput } from "./PickUpDropOffPointUpdateManyWithoutRoutesInput";
import { Type } from "class-transformer";
import { PsvUpdateManyWithoutRoutesInput } from "./PsvUpdateManyWithoutRoutesInput";

@InputType()
class RouteUpdateInput {
  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  distance?: number | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => PickUpDropOffPointUpdateManyWithoutRoutesInput,
  })
  @ValidateNested()
  @Type(() => PickUpDropOffPointUpdateManyWithoutRoutesInput)
  @IsOptional()
  @Field(() => PickUpDropOffPointUpdateManyWithoutRoutesInput, {
    nullable: true,
  })
  pickUpDropOffPoints?: PickUpDropOffPointUpdateManyWithoutRoutesInput;

  @ApiProperty({
    required: false,
    type: () => PsvUpdateManyWithoutRoutesInput,
  })
  @ValidateNested()
  @Type(() => PsvUpdateManyWithoutRoutesInput)
  @IsOptional()
  @Field(() => PsvUpdateManyWithoutRoutesInput, {
    nullable: true,
  })
  psvs?: PsvUpdateManyWithoutRoutesInput;
}

export { RouteUpdateInput as RouteUpdateInput };
